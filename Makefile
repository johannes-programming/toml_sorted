...
test:
	conda run -n base python make/env.py test_toml_sorted --python=3.11 --recreate >/dev/null;
	conda run -n test_toml_sorted pip install -e . >/dev/null;
	conda run -n test_toml_sorted python run_tests.py;
	conda run -n test_toml_sorted pip install mypy >/dev/null;
	conda run -n test_toml_sorted python -m mypy --strict .;
	conda run -n test_toml_sorted python -m mypy --strict -p toml_sorted;
